...
extrusion_width = 19.94;
roller_dia = 15.96;  // Measured diameter of 3 different Grabercars double 623 w-wheels.
...
m3_nut_slop_v = 0.2;  // Account for inability for layer height to exactly match nut width.
m3_nut_slop_h = 0.8;  // Account for holes in the Z plane coming out too small.
m3_nut_dia_v = 6.18 + m3_nut_slop_v;
m3_nut_dia_h = 6.18 + m3_nut_slop_h;
m3_nut_r_v = m3_nut_dia_v / 2;
m3_nut_r_h = m3_nut_dia_h / 2;
...
m3_screw_head_slop = 0.8;
m3_screw_head_dia = 5.5 + m3_screw_head_slop;
m3_screw_head_r = m3_screw_head_dia/2;
...
module cut()
{
  ...
  translate([cut_offset_x+minimal_cut/2, -main_cube_length/8, 0]) {
    translate([0,-5,0]) cube([cut_width, main_cube_length/4+cut_width + 10, main_height + 2], center = true);
    translate([0, 1, 0]) rotate([0, 90, 0]) {
      cylinder(r=m3_screw_r, h=100, $fn=smooth, center = true);
    }
  }
  // Nut trap for tensioning screw
  translate([-main_cube_width/2-delta+m3_nut_thickness/2+m3_nut_thickness_extra_tensioner/2, 1-main_cube_length/8, 0]) {
    rotate([30, 0, 0]) rotate([0, 90, 0]) {
      cylinder(r=m3_nut_r_h, h=m3_nut_thickness+delta+m3_nut_thickness_extra_tensioner, $fn=6, center=true);
    }
  }
  // Cut to outer edge of part, along x
  translate([main_cube_width/4+rest_cut+2, -main_cube_length/4-5, 0]) {
    cube([rest_cut, cut_width+10, main_height + 2], center = true);
  }
}
    ...
    difference() {
      ...
      difference() {
        ...
        translate([-main_cube_width/4-delta, -delta, -main_height/2])
          cube([6+delta, 17+delta, main_height]);
      }
      ...
      translate([0, 2.5, 0]) {
        ...
        translate([10, -10, -main_height/2-delta])
          cylinder(r=m3_nut_r_v, h=m3_nut_thickness_extra+delta, $fn=6);
        for (i=[-1, 1]) {
